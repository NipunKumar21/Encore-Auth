...
export async function generateTokens(userID:string): Promise<{
    ...
    const accessToken =await new SignJWT({userID,type:"access"}).setProtectedHeader({alg:"HS256"}).setExpirationTime(ACCESS_TOKEN_EXPIRATION).sign(new TextEncoder().encode(JWT_SECRET()));
